...
//
// Both can be armed at once, but not at this interval. Tail retention only
// fires when more events accumulate between sliding-window compactions than
// EventRetentionSize holds back, and an interval of 2 keeps that number tiny.
// This example uses 2 so a compaction happens within a few turns of starting
// it, which is the wrong end of that trade for tail retention. Raise the
// interval and both run; the package documentation has the measurements.
//
...
func main() {
	...
	config := &launcher.Config{
		...
		EventsCompactionConfig: &compaction.Config{
			...
			// Tail retention, commented out on purpose. Arming it alongside
			// the interval of 2 above would do nothing: the sliding window
			// summarizes every group of two turns, so the events this trigger
			// looks at, the ones no compaction covers yet, never reach
			// EventRetentionSize and it never fires.
			//
			...
			// just reducing it. Running both is fine at a larger interval.
			//
