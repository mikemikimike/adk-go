...
//
// Not that the prompt is smaller. On a three-turn recording it is not: at the
// compaction boundary this one goes from 183 characters to 870, because a
// summary of several short turns is longer than the turns. Compaction pays off
// against real history, and this fixture is not that, so there is no size
// assertion here.
//
...
func TestCompactionE2E(t *testing.T) {
	...
	// No assertion on prompt size here, deliberately.
	//
	// The doc comment used to promise one and there was none. Adding one showed
	// why: at the real compaction boundary this recording goes from 183
	// characters to 870, and that is correct rather than a defect, because a
	// summary of several short turns is longer than the turns. An assertion
	// placed there fails on working code. Placed anywhere else it compares two
	// prompts on the same side of the boundary and cannot fail at all, which is
	// worse: the version that lived here did not fire against the recording
	// that carried a 3,464-byte thought signature, because promptTextOf counts
	// text and a signature is bytes.
	//
	// What the size of a compacted prompt should be is a property of real
	// history, not of a three-turn fixture. The offline tests above assert what
	// this recording can actually establish: the summary is in the prompt and
	// the turns it covers are not.
