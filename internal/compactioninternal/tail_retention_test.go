...
// TestSeedIsTakenFromTheWindowsOwnBranch pins that a sibling branch compacting
// more recently does not stop a branch rolling up its own summary.
//
// The seed keeps history to one rolling summary. Taking the latest record
// across every branch and then refusing it on a scope mismatch looks safe and
// is not the same thing: when a sibling compacted last, the seed is dropped and
// this branch never absorbs its own earlier summary, so it accumulates a chain
// and the bound quietly stops applying. Nothing is lost, which is why it went
// unnoticed.
func TestSeedIsTakenFromTheWindowsOwnBranch(t *testing.T) {
	t.Parallel()
	...
	events := []*session.Event{
		onBranch(textEvent("a1", "invA1", 1, "A one"), "root.A"),
		onBranch(modelTextEvent("a2", "invA1", 2, "A two"), "root.A"),
		onBranch(compactionEvent("SA", 3, 1, 2, "summary of A"), "root.A"),
		onBranch(textEvent("b1", "invB1", 4, "B one"), "root.B"),
		onBranch(compactionEvent("SB", 5, 4, 4, "summary of B"), "root.B"),
		onBranch(textEvent("a3", "invA2", 6, "A three"), "root.A"),
		onBranch(modelTextEvent("a4", "invA2", 7, "A four"), "root.A"),
		onBranch(textEvent("a5", "invA3", 8, "A five"), "root.A"),
	}

	window := selectTailRetentionWindow(events, 1, TurnScope{Branch: "root.A"})
	got := ids(window)
	if !slices.Contains(got, "SA") {
		t.Errorf("window %v does not open with branch A's own summary, so A never rolls it up and grows a chain", got)
	}
	if slices.Contains(got, "SB") {
		t.Errorf("window %v seeded from a sibling branch, merging across the scope boundary", got)
	}
}

// TestTailRetentionWindowStaysInsideTheTurnsScope pins that the window is drawn
// from the same conversation whose size triggered the pass.
//
// promptTokenCount measures only what the turn can see, and its comment gives
// the reason: "a sub-agent whose own prompt is a couple of tokens read its
// parent's 200,000 and compacted history it had no business compacting". The
// candidate list did not apply that filter, so the threshold was evaluated
// against one conversation and the window chosen from another. A sub-agent
// spent its one allowed compaction summarizing a sibling branch, its own prompt
// did not shrink by a token, and the progress gate then stood down for the rest
// of an over-threshold turn -- the strategy that bounds growth, switched off by
// having compacted the wrong thing.
func TestTailRetentionWindowStaysInsideTheTurnsScope(t *testing.T) {
	t.Parallel()
	...
	events := []*session.Event{
		onBranch(textEvent("b1", "invB", 1, "the sibling's only event"), "root.B"),
		onBranch(textEvent("a1", "invA1", 2, "A one"), "root.A"),
		onBranch(modelTextEvent("a2", "invA1", 3, "A two"), "root.A"),
		onBranch(textEvent("a3", "invA2", 4, "A three"), "root.A"),
		onBranch(modelTextEvent("a4", "invA2", 5, "A four"), "root.A"),
	}

	got := ids(selectTailRetentionWindow(events, 2, TurnScope{Branch: "root.A"}))

	if len(got) == 0 {
		t.Fatal("selectTailRetentionWindow() = empty, want root.A's own eligible events")
	}
	if slices.Contains(got, "b1") {
		t.Errorf("window %v summarizes a sibling branch: root.A's prompt does not shrink, and the compaction it was allowed is spent", got)
	}
	if diff := cmp.Diff([]string{"a1", "a2"}, got); diff != "" {
