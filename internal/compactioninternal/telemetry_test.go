...
// TestDeclinedSummarizationReportsWhatItSpent pins that a decline is visible on
// the span, with the usage the summarizer reported.
//
// Three places disagreed about this and the code followed the one that did
// nothing: the interface invites a summarizer to report usage alongside a
// decline, the compactor's comment said the span records it, and the span was
// left bare. So a decline looked identical to a compaction that was never
// attempted, and a summarizer that spent a model call and got nothing usable
// back reported no spend at all.
func TestDeclinedSummarizationReportsWhatItSpent(t *testing.T) {
	// Not parallel: spanRecorder overrides the global tracer, so every test in
	// this file that records spans has to run alone. It is the only one here
	// that was.
	...
	cfg := &compaction.Config{
		...
		Summarizer: &usageReportingDecliner{usage: &genai.GenerateContentResponseUsageMetadata{
			PromptTokenCount: 120, CandidatesTokenCount: 15,
		}},
	}
	if _, err := slidingWindowStored(context.Background(), cfg, &staticSession{events: events}); err != nil {
		t.Fatalf("slidingWindowStored() error = %v", err)
	...
	if _, ok := got["gen_ai.compaction.declined"]; !ok {
		t.Error("a decline left no marker on the span, so it is indistinguishable from compaction never being attempted")
	}
	if _, ok := got["gen_ai.usage.input_tokens"]; !ok {
		t.Error("the span records no input tokens for a decline that reported usage")
	}
	if _, ok := got["gen_ai.usage.output_tokens"]; !ok {
		t.Error("the span records no output tokens for a decline that reported usage")
	}
}

// usageReportingDecliner declines while reporting what the attempt cost.
type usageReportingDecliner struct {
	usage *genai.GenerateContentResponseUsageMetadata
}

func (d *usageReportingDecliner) SummarizeEvents(context.Context, []*session.Event) (*genai.Content, *genai.GenerateContentResponseUsageMetadata, error) {
	return nil, d.usage, nil
}

// TestFailedSummarizationReportsWhatItSpent pins the same property for a
// failure, which is the case that costs the most.
//
// The built-in summarizer returns usage alongside the error it raises for a
// MAX_TOKENS or safety stop, having already paid for a full-transcript prompt.
// Usage was recorded after the early return taken for an error, so a session
// failing every compaction reported no spend at all while burning the largest
// prompts it ever sends.
func TestFailedSummarizationReportsWhatItSpent(t *testing.T) {
	// Not parallel: spanRecorder overrides the global tracer.
	...
	cfg := &compaction.Config{
		CompactionInterval: 2, OverlapSize: 1,
		Summarizer: &usageReportingFailer{usage: &genai.GenerateContentResponseUsageMetadata{
			PromptTokenCount: 4000, CandidatesTokenCount: 8,
		}},
	}
	if _, err := slidingWindowStored(context.Background(), cfg, &staticSession{events: events}); err == nil {
		t.Fatal("slidingWindowStored() error = nil, want the summarizer's failure")
	}
	...
	got := attrs(spans[0].Attributes)
	if _, ok := got["gen_ai.usage.input_tokens"]; !ok {
		t.Error("the span records no input tokens for a failure that reported usage, so the spend is invisible")
	}
	if _, ok := got["gen_ai.usage.output_tokens"]; !ok {
		t.Error("the span records no output tokens for a failure that reported usage")
	}
	if _, ok := got["gen_ai.compaction.result_event_id"]; ok {
		t.Error("a failed compaction named a result event, so the span is at once an error and a success")
	}
}

// usageReportingFailer fails while reporting what the attempt cost, as the
// built-in summarizer does for a MAX_TOKENS or safety stop.
type usageReportingFailer struct {
	usage *genai.GenerateContentResponseUsageMetadata
}

func (f *usageReportingFailer) SummarizeEvents(context.Context, []*session.Event) (*genai.Content, *genai.GenerateContentResponseUsageMetadata, error) {
	return nil, f.usage, errors.New(`summarizer stopped before finishing (finish reason "MAX_TOKENS"), so the summary is incomplete`)
}
